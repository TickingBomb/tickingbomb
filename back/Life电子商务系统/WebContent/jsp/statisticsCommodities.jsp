...
<body background="#F3F3F3">
	...
	<form action="statisticsCommoditiesWithPageview.do?state=2"
		method="post">
		<input type="submit" value="商品访客排行" class="a2" />
	</form>
	<br>
	<form class="so" action="selectByCondition.do?state=2" method="post"
		id="myform">
		...
		</select> 开始时间：<input type="time" name="starttime" id="starttime" value="${pageBean.starttime}"/> 结束时间：<input type="time"
			name="closetime" id="closetime" value="${pageBean.closetime}"/> <select>
			<option>商品编号</option>
		</select> <input type="text" name="selectInfo" id="selectInfo" value="${pageBean.selectInfo}"> <input
			type="button" value="搜索" class="ch_1" onclick="fun()"> <input
			type="button" class="ch_1" onclick="resetAll()" value="重置">
		<table border="1" width="100%" cellpadding="1" cellspacing="0"
			...
			<c:forEach items="${comlist}" var="temp" varStatus="state">
				<tr>
					<th>${state.count }</th>
					<th>${temp.commodity_no }</th>
					<th>${temp.introduce }</th>
					<th>${temp.acount }</th>
					<th>${temp.listprice }</th>
				</tr>
			</c:forEach>
		</table><br>
		<div class="div_info_all">
			<div class="div_input">
				...
				<input type="button" class="input_scan" value="末页"
					onclick="page(${pageBean.totalPage -1})" /> <span
					class="span_text">跳到第</span> <input type="text" class="input_text"
					name="pageNo" id="pageID" /> <span class="span_text">页</span> <input
					type="button" class="input_scan" value="跳转"
					onclick="pageSubmit(${pageBean.totalPage})" />
			</div>
